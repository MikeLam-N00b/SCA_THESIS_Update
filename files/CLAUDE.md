...
## Active Development: Friend Sharing

**Status hiện tại:**
- ✅ Server: endpoints `/friend-sharing/*` hoàn chỉnh
- ✅ Android models: `FriendKeyBundle`, `FriendShareRequest/Response`
- ✅ Android network: `ApiService.createFriendShare`, `claimFriendShare`
- ❌ Android UI: chưa có Fragment cho friend sharing flow
- ❌ Firmware Anchor: chưa có logic xử lý friend key

**Khi implement firmware friend sharing:**
```c
// Pattern bắt buộc: verify ECDSA sig TRƯỚC khi dùng friend_key
// 1. Anchor nhận friend bundle qua BLE từ Tag (friend)
// 2. Parse: friend_id, vehicle_id, friend_key_hex, expires_at, sig
// 3. Verify: server public key + ECDSA(SHA256) trên message
// 4. Check: expires_at chưa quá hạn
// 5. Nếu valid: dùng friend_key cho HMAC challenge thay pairingKey
if (!verifyFriendBundleSig(bundle)) {
    Serial.println("[FRIEND] Invalid sig — rejected");
...
SCA/
├── Src/FreeRTOS/
│   ├── FreeRTOS_Anchor_TestSimFetchKey/   # Anchor firmware (MAIN)
│   │   ├── FreeRTOS_Anchor_TestSimFetchKey.ino
│   │   ├── anchor_config.h                # PIN defs, UUIDs, VEHICLE_ID
│   │   ├── can_commands.h                 # CANCommands class
│   │   └── can_frames.h                   # CAN frame definitions
│   └── FreeRTOS_Tag/                      # Tag firmware (MAIN)
│       ├── FreeRTOS_Tag.ino
│       └── tag_config.h
├── Server/
│   ├── main.py                            # FastAPI (production)
│   ├── main_2.py                          # Experimental
│   ├── car_access.db                      # SQLite
│   ├── anchor_client.py                   # Test: Anchor simulator
│   ├── tag_client.py                      # Test: Tag simulator
│   └── friend_client.py                   # Test: Friend flow
├── AndroidApp/app/src/main/java/com/example/uwb/
├── lib/
│   ├── Dw3000/                            # UWB driver library
│   └── autowp-mcp2515/                    # CAN driver library
├── example/                               # Prototype code (reference only)
└── Tools/
    ├── nvs_clear.py                       # Xóa NVS flash
