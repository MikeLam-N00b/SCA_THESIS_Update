        ...
        if (xQueueReceive(bleQueue, &msg, pdMS_TO_TICKS(200)) == pdTRUE) {
            ...
            case BLE_FETCH_KEY: {
                ...
                if (myGen != (uint8_t)connectionGen || !deviceConnected) {
                    Serial.println("[BLE] BLE_FETCH_KEY: stale or disconnected — skip");
                    break;
                }
                Serial.println("[BLE] Fetching pairing key from server...");
                if (WiFi.status() != WL_CONNECTED) {
                    if (!wifiInit()) {
                        Serial.println("[BLE] WiFi connect failed — cannot fetch key, disconnecting Tag");
                        if (pBleServer && deviceConnected)
                            pBleServer->disconnect(pBleServer->getConnId());
                        break;
                    }
                }
                // Check again after WiFi init — Tag may have disconnected
                if (myGen != (uint8_t)connectionGen || !deviceConnected) {
                    Serial.println("[BLE] Tag disconnected during WiFi init — skip");
                    break;
                }
                char newKey[33];
                if (fetchPairingKeyViaWifi(newKey)) {
                    saveKeyToNVS(newKey);
                    fetchServerSigningKey();
                    hexStringToBytes(bleKeyHex, pairingKey, 16);
                    printHex("[BLE] Pairing key fetched: ", pairingKey, 16);
                    // Now send challenge as normal
                    BleCmdMsg challengeMsg = {};
                    challengeMsg.type    = BLE_SEND_CHALLENGE;
                    challengeMsg.data[0] = connectionGen;
                    xQueueSend(bleQueue, &challengeMsg, pdMS_TO_TICKS(10));
                } else {
                    Serial.println("[BLE] Key fetch failed — disconnecting Tag");
                    if (pBleServer && deviceConnected)
                        pBleServer->disconnect(pBleServer->getConnId());
                }
                break;
