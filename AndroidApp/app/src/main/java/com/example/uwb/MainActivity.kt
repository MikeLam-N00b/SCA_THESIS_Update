...
class MainActivity : AppCompatActivity(), NfcAdapter.ReaderCallback {
    ...
    override fun onTagDiscovered(tag: Tag?) {
        ...
        try {
            ...
            if (finalVin != null) {
                Log.d("SCA_NFC", "VIN parsed successfully")
                runOnUiThread {
                    android.widget.Toast.makeText(this, "VIN Scanned: $finalVin", android.widget.Toast.LENGTH_SHORT).show()
                }
                handleVinParsed(finalVin)
            } else {
                Log.e("SCA_NFC", "VIN not found in NFC payload")
            }

            if (finalKey != null) {
                Log.d("SCA_NFC", "NFC KEY parsed successfully")
                try {
                    val keyBytes = hexToByteArray(finalKey)
                    KeyManager.savePairingKey(finalVin ?: "UNKNOWN_VIN", "NFC_TAG", keyBytes)
                    Log.d("SCA_NFC", "KEY stored locally")
                    Log.d("SCA_NFC", "TAG key is ready for BLE authentication")
                    runOnUiThread {
                        android.widget.Toast.makeText(this, "TAG - Entered KEY", android.widget.Toast.LENGTH_LONG).show()
                    }
                } catch (e: Exception) {
                    Log.e("SCA_NFC", "Error parsing KEY hex", e)
                    runOnUiThread {
                        android.widget.Toast.makeText(this, "TAG - KEY format error", android.widget.Toast.LENGTH_SHORT).show()
                    }
                }
            } else {
                Log.e("SCA_NFC", "KEY not found in NFC payload")
                runOnUiThread {
                    android.widget.Toast.makeText(this, "TAG - KEY not found", android.widget.Toast.LENGTH_SHORT).show()
                }
            }
